...
The envionment requires a shared Azure front door profile created in the hub. The service name must be declared in [the hub configuration](https://github.com/NHSDigital/dtos-hub/tree/main/infrastructure/environments). And run the Azure devops pipeline for the corresponding hub (non-live or live).
...
- For entra ID authentication (when `enable_auth` is true): aad-client-audiences, aad-client-id, aad-client-secret
