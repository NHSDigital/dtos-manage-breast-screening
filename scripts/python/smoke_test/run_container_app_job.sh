...
ENV_VARS="REPORTS_CONTAINER_NAME=notifications-smoke-test-reports"

echo Starting job "$JOB_NAME"...
execution_name=$(az containerapp job start --name "$JOB_NAME" --resource-group "$RG_NAME" --env-vars "$ENV_VARS" | jq -r '.id|split("/")[-1]')
