...
test-unit: # Run unit tests @Testing
	uv run pytest -n auto --dist=load -p no:randomly -m 'not system' --ignore scripts/python/smoke_test --cov --cov-report term-missing:skip-covered --junit-xml=reports/test-python-units.xml
	npm test -- --coverage
...
test-ui: # Run UI tests @Testing
	uv run pytest -n auto --dist=load -p no:randomly -m system --junit-xml=reports/test-ui.xml
