...
test-unit: # Run unit tests @Testing
	uv run pytest -m 'not system' --ignore manage_breast_screening/notifications/tests/dependencies --ignore manage_breast_screening/notifications/tests/integration --ignore manage_breast_screening/notifications/tests/end_to_end --cov --cov-report term-missing:skip-covered
	npm test -- --coverage
...
seed-demo-data:
	uv run ./manage.py seed_demo_data $(ARGS)

models:
	uv run ./manage.py shell -c "from django.apps import apps; print('\n'.join(f'{m._meta.app_label}.{m.__name__}' for m in apps.get_models()))"
...
.DEFAULT_GOAL := help
.PHONY: clean config dependencies build deploy githooks-config githooks-run help test test-unit test-lint test-ui run _install-uv _clean-docker rebuild-db db migrate seed shell
.SILENT: help run
