...
def clinic_list(request, filter="today"):
    ...
    clinics = clinic_repo.by_filter(filter).prefetch_related("setting")
...
def clinic(request, pk, filter="remaining"):
    ...
    clinic = get_object_or_404(
        clinic_repo.get_queryset().select_related("setting"), pk=pk
    )
    presented_clinic = ClinicPresenter(clinic)

    appointment_repo = AppointmentRepository(request.current_provider)
    appointments = appointment_repo.for_clinic_and_filter(clinic, filter).order_by(
...
def check_in(request, pk, appointment_pk):
    appointment_repo = AppointmentRepository(request.current_provider)
    appointment = get_object_or_404(appointment_repo.get_queryset(), pk=appointment_pk)
