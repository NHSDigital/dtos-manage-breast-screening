...
class TestCIS2BackChannelLogout(SystemTestCase):
    ...
    def around_each(self):
        ...
        self._requests_mocker.get(
            settings.CIS2_SERVER_METADATA_URL,
            json={"issuer": self.issuer, "jwks_uri": jwks_url},
            status_code=200,
        )

        # 3. Mock the CIS2 auth server JWKS endpoint
        ...
        # Remove the cached oauth client from the registry to avoid leaking state between tests
