...
def cis2_back_channel_logout(request):
    ...
    if settings.DEBUG:
        key_loader = settings.CIS2_PUBLIC_KEY
    else:
    ...
    try:
        verification_rules = {
            ...
            "events": {
                "essential": True,
                "validate": lambda c, v: isinstance(v, dict)
                and "http://schemas.openid.net/event/backchannel-logout" in v,
            },
            "nonce": {"validate": lambda c, v: v is None},
        }
        claims = jwt.decode(
            logout_token,
            key=key_loader,
