...
class TestNhsMail:
    ...
    def test_sends_to_multiple_emails(self, mock_smtp_server, csv_data, monkeypatch):
        ...
        subject = NhsMail()
        subject.send_reports_email({"filename.csv": csv_data})
    ...
    def test_sends_a_reports_email(self, mock_smtp_server, csv_data):
        subject = NhsMail()
        subject.send_reports_email(
            {"filename1.csv": csv_data, "filename2.csv": csv_data}
        )
        ...
        assert (
            "[TEST] Breast screening digital comms reports – 11-10-2025 – Birmingham (MCR)"
            in decoded_subject_line
        )
        assert "Content-Type: text/html" in email_content

        assert "filename1.csv" in email_content
        assert "filename2.csv" in email_content
        for index in [1, 2]:
            decoded_attachment_data = (
                mime_message.get_payload()[index]  # type: ignore
    ...
    def test_does_not_add_environment_name_in_production(
        ...
        decoded_subject_line = str(make_header(decode_header(mime_message["Subject"])))
        assert (
            "Breast screening digital comms reports – 11-10-2025 – Birmingham (MCR)"
            in decoded_subject_line
        )
