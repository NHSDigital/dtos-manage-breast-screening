...
resource groupKvSecretsUserAssignment 'Microsoft.Authorization/roleAssignments@2022-04-01' = if (!empty(groupPrincipalId)) {
  name: guid(subscription().subscriptionId, groupPrincipalId, 'kvSecretsUser')
  properties: {
    roleDefinitionId: subscriptionResourceId('Microsoft.Authorization/roleDefinitions', roleID.kvSecretsUser)
    principalId: groupPrincipalId
    principalType: 'Group'
    description: '${groupName} kvSecretsUser access to resource group'
  }
}
